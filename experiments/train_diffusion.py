...
from experiments.helpers import (
    create_diffusion_model_and_optimizer,
    gather_distributed_losses,
    gather_distributed_metrics,
    get_time_ranges,
    load_autoencoder,
    load_diffusion_surrogate,
    save_checkpoint,
    setup_diffusion_run_config,
...
def train_diffusion_epoch(
    ...
    for step in range(total_steps):
        ...
        with context:
            # Note: For diffusion, loss_fn usually handles the forward pass internally
            loss = loss_fn(model, x, mask=mask, label=label)
...
def validate_diffusion_epoch(
    ...
    """Run diffusion validation epoch."""
    model.eval()
    losses = []
...
def train(
    ...
    """Train a diffusion model for probabilistic surrogate modeling."""
    ...
    surrogate = load_diffusion_surrogate(cfg, device, target)

    # Setup run configuration
    runname, runpath = setup_diffusion_run_config(cfg, runid, space)
    ...
    with open_dict(cfg):
        ...
        if surrogate is not None:
            # Get AE run from surrogate config if available
            ...
            if surrogate_cfg.ae_run:
                ae_run_path = surrogate_cfg.ae_run.replace("/frozet", "/polymathic")
    ...
    denoiser, denoiser_loss, optimizer, scheduler = (
        create_diffusion_model_and_optimizer(
    ...
    for epoch_idx in epochs:
        # Train
        losses, grads = train_diffusion_epoch(
            denoiser,
            train_loader,
            optimizer,
            denoiser_loss,
        ...
        # Rollout evaluation
        if (
            ((epoch_idx % cfg.val_eval.interval == 0) and cfg.val_eval.enabled)
            ...
            for split in rollout_splits:
                ...
                split_logs = run_rollout_evaluation(
                    ...
                    is_diffusion=True,
                    sampling_config=cfg.val_eval.sampling,
        ...
        # Validation
        val_losses = validate_diffusion_epoch(
            denoiser, valid_loader, denoiser_loss, cfg, device, preprocess
        )
...
if __name__ == "__main__":
    ...
    if args.single_gpu:
        ...
        dawgz.schedule(
            ...
            name=f"training diffusion {runid}",
            backend=backend,
